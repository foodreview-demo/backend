...
import com.foodreview.global.exception.ErrorCode;
...
public class ImageService {
    ...
    public ImageDto.UploadResponse uploadImages(List<MultipartFile> files) {
        if (files == null || files.isEmpty()) {
            throw new CustomException(ErrorCode.INVALID_INPUT_VALUE, "업로드할 이미지가 없습니다");
        }
    ...
    private void validateFile(MultipartFile file) {
        if (file.isEmpty()) {
            throw new CustomException(ErrorCode.INVALID_INPUT_VALUE, "빈 파일은 업로드할 수 없습니다");
        }
    ...
    public void deleteImage(String imageUrl) {
        ...
        try {
            ...
            throw new CustomException(ErrorCode.INTERNAL_SERVER_ERROR, "이미지 삭제에 실패했습니다");
        }
    }
    ...
    private String extractKeyFromUrl(String url) {
        ...
        throw new CustomException(ErrorCode.INVALID_INPUT_VALUE, "유효하지 않은 이미지 URL입니다");
    }
}
