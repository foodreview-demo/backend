...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    // 지역별 리뷰 조회 (최신순)
    @Query("SELECT r FROM Review r WHERE r.restaurant.region = :region ORDER BY r.createdAt DESC")
    ...
    // 지역 + 카테고리 필터
    @Query("SELECT r FROM Review r WHERE r.restaurant.region = :region AND r.restaurant.category = :category ORDER BY r.createdAt DESC")
